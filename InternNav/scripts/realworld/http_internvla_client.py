...
def dual_sys_eval(image_bytes, depth_bytes, front_image_bytes, url='https://nat2-notebook-inspire.sii.edu.cn/ws-6040202d-b785-4b37-98b0-c68d65dd52ce/project-aa6664a6-c21b-426f-b8b9-f3d84953ccff/user-aaf54b73-20ec-4036-a290-167b1935d31d/vscode/9806885b-f12b-4b38-b57a-968a9f7f1c36/c1a34928-e77e-48fe-8f0e-2687c00b5fac/proxy/5801/eval_dual'):
    ...
    curr_w = manager.angular_vel if manager else 0.0
    odom_rw_lock.release_read()
    data = {"reset": policy_init, "idx": http_idx, "v": round(curr_v, 3), "w": round(curr_w, 3)}
...
class Go2Manager(Node):
    def __init__(self):
        ...
        self.get_logger().info("=== Go2Manager initializing ===")

        self.get_logger().info("Subscribing to /camera/camera/color/image_raw (RGB)")
        rgb_down_sub = Subscriber(self, Image, "/camera/camera/color/image_raw")
        self.get_logger().info("Subscribing to /camera/camera/aligned_depth_to_color/image_raw (depth)")
        depth_down_sub = Subscriber(self, Image, "/camera/camera/aligned_depth_to_color/image_raw")

        qos_profile = QoSProfile(reliability=ReliabilityPolicy.BEST_EFFORT, history=HistoryPolicy.KEEP_LAST, depth=10)

        self.get_logger().info("Setting up ApproximateTimeSynchronizer (RGB+depth, slop=0.1s)")
        ...
        self.get_logger().info("Subscribing to /odom_bridge (odometry)")
        self.odom_sub = self.create_subscription(Odometry, "/odom_bridge", self.odom_callback, qos_profile)

        # publisher
        self.get_logger().info("Publishing to /cmd_vel_bridge (velocity commands)")
