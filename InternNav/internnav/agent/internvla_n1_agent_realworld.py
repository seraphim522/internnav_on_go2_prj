...
class InternVLAN1AsyncAgent:
    ...
    def step(self, rgb, depth, pose, instruction, intrinsic, look_down=False, v_curr=0.0, w_curr=0.0):
        ...
        if (self.episode_idx - self.last_s2_idx > self.PLAN_STEP_GAP) or look_down or no_output_flag:
            self.output_action, self.output_latent, self.output_pixel = self.step_s2(
                rgb, depth, pose, instruction, intrinsic, look_down, v_curr, w_curr
            )
    ...
    def step_s2(self, rgb, depth, pose, instruction, intrinsic, look_down=False, v_curr=0.0, w_curr=0.0):
        image = Image.fromarray(rgb).convert('RGB')
        if not look_down:
            image = image.resize((self.resize_w, self.resize_h))
            self.rgb_list.append(image)
        ...
        # 注入当前速度信息到 prompt，让 LLM 知道机器人运动状态
        if abs(v_curr) < 0.03:
            moving_text = "stationary"
        elif v_curr > 0:
            moving_text = f"moving forward at {v_curr:.2f} m/s"
        else:
            moving_text = f"moving backward at {abs(v_curr):.2f} m/s"
        if abs(w_curr) > 0.05:
            turning_text = f"turning {'left' if w_curr > 0 else 'right'}"
        else:
            turning_text = "not turning"
        sources[0]["value"] += f" You are currently {moving_text}, {turning_text}."
